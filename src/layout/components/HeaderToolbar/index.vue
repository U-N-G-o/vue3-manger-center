  ...
  <el-header :height="
    ...
    <div class="headerToolbar-toolbox">
      <div class="headerToolbar-toolbox__left">
        <svg-icon :name="collapse ? 'open-menu' : 'hide-menu'" size="2xl" pointer
      ...
      <div class="headerToolbar-toolbox__right">
        ...
        <el-tooltip :content="fullscreen ? '退出全屏' : '全屏'" placement="bottom">
          <svg-icon :name="fullscreen ? 'fullscreen-exit' : 'fullscreen'" size="2xl" pointer
        ...
        <svg-icon name="user" />
        &emsp;
        <el-dropdown trigger="click" placement="bottom-start" @command="$emit('dropCommand', $event)">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item v-for="item in dropdownMenu" :key="item.key" :command="item.key"
    ...
    <div class="headerToolbar-tabbox" @mouseover="onTabBoxHover('over')" @mouseleave="onTabBoxHover('leave')">
      <tags-view :style="{ transform: `translateX(${tagsViewScroll}%)` }" @mouseover.stop @mouseleave.stop />
    </div>
  </el-header>
  <tags-pager v-model:visable="tagsPager.show" :root-style="{ top: tagsPager.top + 'px', left: tagsPager.left + 'px' }"
    @hover="onTabBoxHover" @arrow-click="tagsPager.onArrowClick" />
</template>

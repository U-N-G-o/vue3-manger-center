<template>
  <div v-fullscreen="fullscreen" class="wrapper">
    <el-container class="wrapper-container">
      <aside-bar :collapse="collapse" :icon-name="Setting.ICON_NAME" :icon-color="Setting.ICON_COLOR"
        :system-name="Setting.SYSTEM_NAME" />
      <el-container class="wrapper-container__right">
        <header-toolbar username="admin" :dropdown-menu="Setting.DROP_DOWN_LIST" @drop-command="onDropCommand"
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const onDropCommand = (command: DropCommand) => {
        switch (command) {
          case 'github':
            window.open('https://github.com/U-N-G-o/vue3-manger-center', '_blank')
            break
          case 'logout':
            window
              .$confirm({
      ...
      const showWelcome = () => {
        ...
        if (isLogin && firstLoaded.value) {
          window.$notify('欢迎', 'admin', 'success')
